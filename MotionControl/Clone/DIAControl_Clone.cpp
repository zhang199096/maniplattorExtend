...
// 				}
	...
	//			SetEvent(event_DualCore_Clone_send);
			...
			//KillTimer(nIDEvent);			
			//SetTimer(nIDEvent, 20, NULL);
			if ((OverallSign_Clone.packsendflag & 0x00000001) == 0x00000001)
			{
				OverallSign_Clone.packsendflag = (OverallSign_Clone.packsendflag & 0xFFFFFFFE);
				if (OverallSign_Clone.PC_DSP_COM_OK)
				{
					DSP_com_MainCMD_b0();
					//printf("DualCore_Clone b0 trigger event\n");
...
void DualCore_UDP_ResendDeal(void)
{
	...
	if (UDP_DualCore_WaitRespFlag > 0)
	{
		GetLocalTime(&st_udpdualcore_resend);

		st_udpdualcore_curms = st_udpdualcore_resend.wSecond * 1000 + st_udpdualcore_resend.wMilliseconds;
		intertimer = st_udpdualcore_curms - st_udpdualcore_lastms;
		if (intertimer > 100)
		{
			udpdualcore_resend_times++;
			if (udpdualcore_resend_times > 3)
			{
				//Multpump_WaitRespFlag = 0;
				udpdualcore_resend_times = 0;
				SetEvent(event_DualCore_UDP_Rsend);
				return;
			}
			temp = UDP_DualCoreSendData((char*)DualCore_ResendTxBuf, (DualCore_ResendLength + 4) * 4, IP_Dual_Core);
			if (temp == 0)
			{
				printf("DualCore send fail\n");
				//Multpump_WaitRespFlag = 0;
			...
			printf("DualCore resend udp\n");
			st_udpdualcore_lastms = st_udpdualcore_curms;
		}
		else
		{
			if (intertimer < 0)
